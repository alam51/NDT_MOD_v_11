...
query_str = f"""
SELECT s.id as 'ss_id', s.name AS 'substation', se.id as 'eq_id', se.name AS 'equipment', se.is_bus, se.is_transformer, 
...
for i, (date_time, eq_id) in enumerate(df.index):
    try:
        ...
        print(f'Error in date time : {date_time}')
